...
describe('ContentConverter', () => {
  ...
  describe('validateContent', () => {
    ...
    it('should reject content that is too short', () => {
      ...
      const result = contentConverter.validateContent(html);
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Content is too short (minimum 10 words)');
    });
  ...
  describe('convertHtmlToKindle', () => {
    ...
    it('should successfully convert HTML to EPUB', async () => {
      ...
      const conversionPromise = contentConverter.convertHtmlToKindle(html, mockMetadata);
    ...
    it('should handle Calibre conversion failure', async () => {
      ...
      expect(result.error).toContain('Calibre conversion failed');
    });
    ...
    it('should handle filesystem errors', async () => {
      ...
      const html = '<html><body><p>Test content</p></body></html>';
      const result = await contentConverter.convertHtmlToKindle(html, mockMetadata);
