...
import { validateEmailReceive, validateMailgunSignature } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    await processConversionAsync(conversionId, bodyHtml, metadata, user.kindleEmail);
...
async function processConversionAsync(
  ...
  try {
    ...
    if (!conversionResult?.success) {
      throw new Error(conversionResult?.error || 'Conversion failed after retries');
    }
